...
>(({ className, ...props }, ref) => {
    const { toggleSidebar, setSidebarWidth, open, setOpen } = useSidebar();
    ...
    const handlePointerDown = React.useCallback(
        (e: React.PointerEvent) => {
            ...
            // Get the current computed sidebar width
            const sidebarEl = (e.currentTarget as HTMLElement).closest('[data-sidebar="sidebar"]');
            startWidth.current = sidebarEl?.getBoundingClientRect().width ?? 256;
            ...
            const handlePointerMove = (moveEvent: PointerEvent) => {
                ...
                if (!isDragging.current && Math.abs(dx) > 3) {
                    ...
                    if (!open) {
                        setOpen(true);
                        startWidth.current = SIDEBAR_MIN_WIDTH;
                    }
                }
                if (isDragging.current) {
                    const newWidth = Math.min(
                        SIDEBAR_MAX_WIDTH,
                        Math.max(SIDEBAR_MIN_WIDTH, startWidth.current + dx),
                    );
            ...
            const handlePointerUp = () => {
                ...
                if (!isDragging.current) {
                    // It was a click, not a drag — toggle the sidebar
                    toggleSidebar();
                }
                isDragging.current = false;
            };
        ...
        [toggleSidebar, setSidebarWidth, open, setOpen],
    );
...
>(({ className, showIcon = false, ...props }, ref) => {
    ...
    return (
        ...
        >
            {showIcon && (
                <div className="flex size-4 items-center justify-center rounded-md bg-sidebar-accent">
                    <div className="size-4 animate-pulse rounded-md" />
                </div>
            )}
            <div
